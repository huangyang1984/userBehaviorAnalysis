    ...
    public JSONObject model2JSON(Object model, Collection jsonAwareCollect) {
        ...
        for (int i = 0; i < methods.length; i++) {
            ...
            if (getObj instanceof LazyInitializer) {
                ...
                getObj = DateTimeUtils.calendar2StrDateTime((Calendar) getObj);
            } else if (getObj instanceof Date) {
                getObj = DateTimeUtils.date2StrDate((Date) getObj);
...
//                    log.info("●●●●●●●●●●●●●●●method:" + model.getClass().getName() + "." + method.getName());
            ...
            try {//测试at net.sf.json.util.CycleDetectionStrategy$StrictCycleDetectionStrategy.handleRepeatedReferenceAsObject(CycleDetectionStrategy.java:73)
