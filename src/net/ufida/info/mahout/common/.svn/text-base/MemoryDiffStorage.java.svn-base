...
package net.ufida.info.mahout.common;
...
public final class MemoryDiffStorage implements DiffStorage {
  ...
  public MemoryDiffStorage(DataModel dataModel,
    ...
    this.refreshHelper = new RefreshHelper(new Callable<Object>() {
      ...
      public Object call() throws TasteException {
        buildAverageDiffs();  //相似度计算
        return null;
      }
    });
    
    refreshHelper.addDependency(dataModel);  //添加依赖刷新接口
  ...
  private long processOneUser(long averageCount, long userID) throws TasteException {
    ...
    PreferenceArray userPreferences = dataModel.getPreferencesFromUser(userID); //���û���������Ʒ
    int length = userPreferences.length();
    for (int i = 0; i < length - 1; i++) {
      ...
      for (int j = i + 1; j < length; j++) {
        ...
        long itemIDB = userPreferences.getItemID(j); //userPreferences����û����й�������Ʒ
        RunningAverage average = aMap.get(itemIDB);
