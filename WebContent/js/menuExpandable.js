...
function initializeMenu(menuId, actuatorId) {
	...
	if(i == 0) menu.style.display = "block";
	
    //if (window.opera) return; // I'm too tired
    
    actuator.parentNode.style.backgroundImage = "url(images/Rarrow.gif)";
    actuator.onclick = function() {
        var display = menu.style.display;
        this.parentNode.style.backgroundImage =
            (display == "block") ? "url(images/Rarrow.gif)" : "url(images/Darrow.gif)";
...
function openMenu(menuId) {
    ...
    if (menu != null) {
        var display = menu.style.display;
        menu.parentNode.style.backgroundImage = "url(images/Darrow.gif)";
        menu.style.display = (display == "block") ? "none" : "block";
    }
}
