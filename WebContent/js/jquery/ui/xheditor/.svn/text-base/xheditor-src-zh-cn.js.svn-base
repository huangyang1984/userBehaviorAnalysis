	...
	this.uploadInit=function(jText,tourl,upext,setFunc)
	{
		...
		if(tourl.substr(0,1)=='!')//自定义上传管理页
		{
			jUpBtn.click(function(){
				...
				var result = window.showModalDialog(_this.settings.uplodImgUrl, window,"dialogWidth:22;dialogHeight:15;status:no;help:no");
				if ( !(typeof result == 'undefined') ) {
					var imgUrl = _this.settings.imgServer + "/" + result;
					$('#xheImgSrc').val(imgUrl);
					
					$('#xheCancel').click(_this.hidePanel);
				}
