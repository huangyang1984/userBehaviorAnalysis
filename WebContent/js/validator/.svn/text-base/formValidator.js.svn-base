...
// [更新日期] 2009-12-09
// [版  本 号] ver3.3
// [最后修改者] 王雪峰 http://www.gbicc.net
// [修正版本号] ver3.3.1
// [备     注] 对于原作者文件基础的修改版本增加了一个小版本号进行了区分 原版本为 ver3.3
//====================================================================================================
...
$.formValidator = 
{
	...
	initConfig : function(controlOptions)
	{
		...
		var validatorgroup = controlOptions.validatorgroup;
		
		//---------begin 王雪峰 修改 结果:能够支持同个页面多个form的验证
		...
		//old:只能使用在一个form里分多个校验组,然后每次触发的第一个校验组的submit操作,
		// 不支持多个form,因为多个form的最终能后submit的条件是不一样的
				...
				if (initConfig.wideword)
				{
					for (var i = 0; i < val.length; i++) 
					{
						if (val.charCodeAt(i) >= 0x4e00 && val.charCodeAt(i) <= 0x9fa5){ 
							len += 2;//update by 2010-09-28
							//oracle数据库——恶心啊 ——现在用的服务器oracle汉字算3字节——赵鸿飞
							//len += 3;
						}else {
							len++;
						}
