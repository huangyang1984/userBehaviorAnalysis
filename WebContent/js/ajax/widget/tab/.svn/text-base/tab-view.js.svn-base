...
var tabclickevent ;
	...
	function showTab(parentId,tabIndex)
	{
		...
		if(activeTabIndex[parentId]>=0){
			...
			img.src = contextName+'ajaximage/tab/tab_right_inactive.gif';
		...
		img.src = contextName+'ajaximage/tab/tab_right_active.gif';
	...
	function tabClick()
	{
		...
		if(tabclickevent == null){
		        Event.stopObserving(this,"click",tabclickevent);
		    }
	...
	function rolloverTab()
	{
		if(this.className.indexOf('tabInactive')>=0){
			...
			img.src = contextName+'ajaximage/tab/tab_right_over.gif';
		}
	...
	function initTabs(mainContainerID,tabTitles,activeTab,width,height,closeButtonArray,additionalTab)
	{
		...
		if(!additionalTab || additionalTab=='undefined'){			
			...
			if(height.length>0){
				...
				// wei.li hidden this :dhtmlgoodies_tabObj[mainContainerID].style.height = height;
			}
		...
		for(var no=0;no<tabTitles.length;no++){
			...
			//wei.li add tabclickevent to prototype
		    if(tabclickevent != null){
		        Event.observe(aTab,"click",tabclickevent);
		    }
			...
			if(closeButtonArray[no]){
				...
				closeButton.src = contextName+'ajaximage/tab/close.gif';
		...
		for(var no=0;no<tabs.length;no++){
			if(tabs[no].className=='dhtmlgoodies_aTab' && tabs[no].parentNode.id == mainContainerID){
				if(height.length>0)//wei.li hidden this:tabs[no].style.height = height;
	...
	//modify by wei.li ��Ϊprototype�����Array bug
	
	function getTabIndexByTitle(tabTitle)
	{
		tabTitle = tabTitle.replace(/(.*?)&nbsp.*$/gi,'$1');
		for(var prop in dhtmlgoodies_tabObj){
			var divs = dhtmlgoodies_tabObj[mainContainerID].getElementsByTagName('DIV');
			for(var no=0;no<divs.length;no++){
				if(divs[no].id.indexOf('tabTab')>=0){
					...
					if(spanTitle == tabTitle){
						...
						return Array(mainContainerID,tmpId[tmpId.length-1].replace(/[^0-9]/g,'')/1);
					}		
				}
			}
		}
